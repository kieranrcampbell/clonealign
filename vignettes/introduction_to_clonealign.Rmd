...
falls below `rel_tol`, the EM algorithm is considered converged.
The maximum number of iterations to acheive this is set using the `max_iter` parameter.

## Multithreaded options

The M-step of the EM algorithm consists of $G$ different optimizations for $G$ genes. These optimizations are performed in parallel using the BiocParallel package. The parallel computations are controlled using the `bp_param` argument. For fine control over this see the `bpparam` function. By default clonealign uses the parameters that are returned by a call to `bpparam()`. To use only e.g. 1 core call `clonealign` with `clonealign(..., bp_param = MulticoreParam(workers = 1))`.
